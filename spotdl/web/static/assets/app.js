...
const state = { preference: 'explicit_only' };
...
function searchHero(compact = false) {
...
function preferenceControl() {
    return `<div class="preference-control" role="group" aria-label="Content preference">${['explicit_only', 'prefer_explicit', 'any'].map(pref => `<button class="pref ${state.preference === pref ? 'active' : ''}" data-pref="${pref}">${pref === 'explicit_only' ? '<b>E</b> ' : ''}${preferenceName(pref)}</button>`).join('')}</div>`;
}
...
async function renderAlbum(id) {
    ...
    try {
        ...
        root.innerHTML = shell(`<section class="page">${emptyState('Album unavailable', error instanceof ApiError ? error.detail : 'Metadata could not be loaded.')}</section>`, '/search');
    }
}
async function renderArtist(id) {
    ...
    try {
        ...
        root.innerHTML = shell(`<section class="page">${emptyState('Artist unavailable', error instanceof ApiError ? error.detail : 'Artist metadata could not be loaded.')}</section>`, '/search');
    }
}
async function renderDownloads(history = false) {
    ...
    const load = async () => {
        try {
            ...
            const area = document.querySelector('#jobs');
            if (area)
                area.innerHTML = `<div class="job-grid">${visible.map(jobCard).join('')}</div>` || emptyState('Nothing here yet', history ? 'Finished jobs will show up here.' : 'Pick an album and its progress will show up here.');
            if (!history && jobs.some(j => !['completed', 'failed', 'cancelled', 'needs_review'].includes(j.state)))
                state.poll = window.setTimeout(load, 1200);
        }
        catch (error) {
            const area = document.querySelector('#jobs');
            if (area)
                area.innerHTML = emptyState('Queue unavailable', error instanceof ApiError ? error.detail : 'Could not load downloads.');
        }
    };
    await load();
}
...
async function render() {
    ...
    if (path.startsWith('/albums/'))
        return renderAlbum(path.split('/')[2]);
    if (path.startsWith('/artists/'))
        return renderArtist(path.split('/')[2]);
    if (path === '/downloads')
        return renderDownloads(false);
    if (path === '/history')
        return renderDownloads(true);
    if (path === '/settings')
...
document.addEventListener('click', async (event) => {
    ...
    const pref = target.closest('[data-pref]');
    if (pref) {
        state.preference = pref.dataset.pref;
        await api.saveSettings(state.preference);
        render();
        return;
    }
    const savePref = target.closest('[data-save-pref]');
    if (savePref) {
        state.preference = savePref.dataset.savePref;
        await api.saveSettings(state.preference);
        render();
        return;
    }
    const download = target.closest('[data-download-album]');
    if (download) {
        ...
        try {
            ...
            window.setTimeout(() => navigate('/downloads'), 350);
        }
        catch {
    ...
    if (cancel) {
        await api.cancel(cancel.dataset.cancelJob);
        render();
        return;
    }
    const retry = target.closest('[data-retry-job]');
    if (retry) {
        await api.retry(retry.dataset.retryJob);
        render();
        return;
    }
});
document.addEventListener('keydown', event => { if (event.key === 'Enter') {
    ...
    if (el.matches('[data-nav][tabindex="0"]'))
        navigate(el.dataset.nav);
} });
window.addEventListener('popstate', render);
(async () => {
    try {
        state.preference = (await api.settings()).content_preference;
    }
    catch {
        state.preference = 'explicit_only';
    }
    await render();
})();
